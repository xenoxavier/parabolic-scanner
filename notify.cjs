...
function richText(c, levels, mention) {
  ...
  if (c.gateVol != null && c.gateVol < 2e6) {
    L.push('');
    L.push(`**Thin book** — only \`${usd(c.gateVol)}\` on Gate in 24h. You may not be able to size into this.`);
  }
  if (c.lsrTaker != null && (c.lsrTaker < 0.85 || c.lsrTaker >= 1.2)) {
    L.push(`_Taker ${c.lsrTaker.toFixed(2)} is an extreme — measured 0.68–0.82x, dumps are LESS likely. Not scored bearish._`);
  }
  ...
  if (levels) {
    fields.push({
      ...
      value:
        ...
        (c.gateVol != null && c.gateVol < 2e6
          ? `\n**thin book** — only \`${usd(c.gateVol)}\` on Gate in 24h` : ''),
      inline: false
    });
  }
